...
def normal(L, A, dev, mean):
    return A/(dev * np.sqrt(2*np.pi)) * np.e**(-0.5 * ((L-mean)/dev)**2)
...
def main():
    ...
    ax1.errorbar(xdata, ydata, yerr=yerror, xerr=xerror, fmt=".", label="Q Factors", color="black")
    ...
    my_func = power_series_3
    colour = "blue"
    label = "Cubic Best Fit Curve"
    popt, pcov = optimize.curve_fit(my_func, xdata, ydata, sigma=yerror, maxfev=10000)
    ...
    ax1.plot(xs, curve, label=label, color=colour)

    residual = ydata - my_func(xdata, *popt)
    ax2.errorbar(xdata, residual, yerr=yerror, xerr=xerror, fmt=".", color=colour)
    ...
    my_func = powerlaw
    colour = "red"
    label = "Power Law Best Fit Curve"
    popt, pcov = optimize.curve_fit(my_func, xdata, ydata, sigma=yerror, maxfev=10000)
    # The best fit values are popt[], while pcov[] tells us the uncertainties.
    start = min(xdata)
    stop = max(xdata)
    ...
    my_func = sine
    colour = "brown"
    label = "Sinusoidal Best Fit Curve"
    popt, pcov = optimize.curve_fit(my_func, xdata, ydata, sigma=yerror, p0=(10000, 1, 1, 1),maxfev=10000)
    # The best fit values are popt[], while pcov[] tells us the uncertainties.
    start = min(xdata)
    stop = max(xdata)
    xs = np.arange(start, stop, (stop - start) / 10000)
    ...
    my_func = quadratic
    colour = "orange"
    label = "Quadratic Best Fit Curve"
    popt, pcov = optimize.curve_fit(my_func, xdata, ydata, sigma=yerror, maxfev=10000)
    # The best fit values are popt[], while pcov[] tells us the uncertainties.
    start = min(xdata)
    stop = max(xdata)
    ...
    fig.savefig("Graphs/Q vs L combined.png")
