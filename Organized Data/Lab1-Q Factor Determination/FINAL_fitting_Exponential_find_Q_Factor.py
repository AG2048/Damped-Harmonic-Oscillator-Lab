...
# filename = "clean_3rd_set_angle_period_data_with_edge_removed.txt"
# filename = "clean_3rd_set_angle_period_data.txt"
# filename = "processed_rounded_data.txt"
filename = "plotting_data_from_46.29_s.txt"  # from ~ +20 deg
# filename = "plotting_data_from_129.5_s.txt"  # from ~ -10 deg
# filename = "amplitude_oscillation_data.txt"
# filename = "Exp_osc_plotting_data_from_34_s.txt"  # amp vs number of osc, around 20 deg
""" 
...
def main():
    ...
    plt.rcParams.update({'font.size': 10.9})
    ...
    """
    PLOT LINEAR"""
    linpopt, linpcov = optimize.curve_fit(linear, xdata, ydata, sigma=yerror)
    # The best fit values are popt[], while pcov[] tells us the uncertainties.

    ########### HERE!!! ##############
    residuals = ydata - linear(xdata, *linpopt)
    ss_res = np.sum(residuals ** 2)
    ss_tot = np.sum((ydata - np.mean(ydata)) ** 2)
    r_squared = 1 - (ss_res / ss_tot)
    print(r_squared)
    ...
    # phi = popt[3]
    # best fit values are named nicely
    linu_a = linpcov[0, 0] ** (0.5)
    linu_b = linpcov[1, 1] ** (0.5)
    # u_C = pcov[2, 2] ** (0.5)
    # u_phi = pcov[3, 3] ** (0.5)
    # uncertainties of fit are named nicely
    ...
    linstart = min(xdata)
    linstop = max(xdata)
    linxs = np.arange(linstart, linstop, (linstop - linstart) / 1000)
    lincurve = linear(linxs, *linpopt)
    print(f"lin A: {lina} +- {linu_a}")
    print(f"lin B: {linb} +- {linu_b}")
    """linear end"""
    ...
    ax1.errorbar(xdata, ydata, yerr=yerror, xerr=xerror, fmt=".", label="Amplitude at Each Oscillation", color="blue")
    ...
    ax1.plot(xs, curve, label="Exponential Curve of Best Fit", color="orange", linewidth=5)
    ax1.plot(linxs, lincurve, label="Line of Best Fit", color="red", linewidth=3)
    ...
    ax1.set_xlabel("Time after initial amplitude, t (s)")
    ax1.set_ylabel("Amplitude of pendulum, A (rad)")
    ax1.set_title("A: Decaying amplitude for 0.447±0.005m long pendulum with initial amplitude 0.3487±0.0009rad")
    ...
    ax2.set_xlabel("Time after initial amplitude, t (s)")
    ax2.set_ylabel("Residual of amplitude, Ar (rad)")
    ax2.set_title("B: Residuals of the exponential best fit curve")
    # Here is where you change how your graph is labelled.
