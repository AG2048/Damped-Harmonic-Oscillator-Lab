...
def quartic(t, a, b, c, d, e):
    return a * t ** 4 + b * t**3 + c*t**2 + d*t ** 1 + e*t**0

def hex(t, a, b, c, d, e, f, g):
    return a * t ** 4 + b * t**3 + c*t**2 + d*t ** 1 + e*t**0 + f*t**5 + g*t**6
...
def main():
    ...
    xerror = data[2]
    yerror = data[3]
    # print(min(abs(xerror / xdata)))
    # print(max(abs(xerror / xdata)))
    # print(max(yerror / ydata))
    # Finished importing data, naming it sensibly.

    ########### HERE!!! ##############

    init_guess = (0.12450561621913908, 0.002380952380945528, 1.791860465098666)
    ...
    ax1.errorbar(xdata, ydata, yerr=yerror, xerr=xerror, fmt=".", label="Average Recorded Periods", color="blue")
    ...
    ax1.plot(xs, curve, label="Quadratic Curve of Best Fit", color="black", linewidth=3)
    ...
    ax1.set_xlabel("Release amplitude, θ (rad)")
    ax1.set_ylabel("Period, T (s)")
    ax1.set_title("A: Dependence of period on release amplitude for 0.447±0.005m long pendulum")
    ...
    ax2.set_xlabel("Release amplitude, θ (rad)")
    ax2.set_ylabel("Residuals of period, Tr (s)")
    ax2.set_title("B: Residuals of the quadratic best fit curve")
    # Here is where you change how your graph is labelled.
